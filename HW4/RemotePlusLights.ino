...
void loop() {
  ...
  int value1 = map(analogRead(senso1), 0, 1023, 0, 20);
  int value2 = map(analogRead(senso2), 0, 1023, 0, 20);
  //Serial.println(value1);
  //Serial.println(value2);
  if(toggleLight == true){
    Serial.println("toggle Light is true");
    if (value1 > value2) {
  ...
  if (irrecv.decode(&results)) {
    ...
    else if (results.value == 0x41BEC03F) {
